...
packages <- c("tidyverse", "data.table", "lubridate", "ggplot2", "ggthemes", "svglite")
...
plot <- ggplot(
  ...
  theme(axis.text.x = element_blank(),
        ...
        text = element_text(size = 13) # text size
  )

ggsave("output/unemployment_rate_czech_regions.svg", plot = plot, device = "svg", width = 5, height = 5)
```
...
plot <- ggplot(
  db_card %>%  group_by(region) %>% summarize(avg_unemp = mean(avg_unemployment)) %>% ungroup(), 
       aes(x = reorder(region, avg_unemp), y = avg_unemp)) +
  geom_bar(alpha = 1, stat = 'identity', fill = 'steelblue') +
  ...
  coord_flip() +
  geom_text(aes(label=paste(round(avg_unemp,1), '%')), hjust = 1.05, color = 'white') +
  theme_classic() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.line.x = element_blank(),
...
ggsave("unemployment_rate_czech_regions.svg", plot = plot, device = "svg", width = 5, height = 5)

```

```{r}
db1 <- db_card %>% group_by(age_card_purchase, region) %>% summarize(amount = n()) %>% ungroup()
db1
```


```{r}
ggplot(db1, aes(age_card_purchase, region, fill = amount)) + 
  geom_tile()
```
